...
const shouldFetchSingle = (state, id) => {
  ...
  const { byId, default.item, selected } = state.product;
  if(id === 'default' && !default.item) {
    // we're looking for the default item, but it hasn't been fetched yet
    return true;
  } else if(id === 'default' && default.isFetching) {
    // the default item is already fetching.  don't do anything.
    return false;
